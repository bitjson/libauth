...
import {
  ...
  isPushOnly,
  isStandardOutputBytecodePre2023,
  isWitnessProgram,
  mapOverOperations,
  op0NotEqual,
  op1Add,
...
export const createAuthenticationProgramStateBch2020 = ({
  program,
  instructions,
  stack,
  metrics = {
  ...
  stack: Uint8Array[];
  metrics?: AuthenticationProgramStateCommon['metrics'];
...
/**
 * create an instance of the BCH 2020 virtual machine instruction set.
 *
 ...
 */
export const createInstructionSetBch2020 = (
  standard = true,
  {
    ripemd160,
    secp256k1,
...
> => {
  ...
  return {
    ...
    evaluate: (program, { stateEvaluate }) => {
      ...
      const initialState = createAuthenticationProgramStateBch2020({
        instructions: unlockingInstructions,
        program,
        stack: [],
        transactionLengthBytes: encodeTransactionBch(program.transaction)
          .length,
      });
    ...
    verify: ({ sourceOutputs, transaction }, { evaluate, success }) => {
      ...
      if (standard) {
        ...
        for (const [index, output] of sourceOutputs.entries()) {
          if (!isStandardOutputBytecodePre2023(output.lockingBytecode)) {
